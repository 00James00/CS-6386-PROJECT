...
def pbt_with_population_size(population_size, max_time_limit, component_idx, f1_score_thresholds):
    global_start = time.time()
    time_limit_reached = False
    current_epoch = 0

   
    f1_iterations = {f1: None for f1 in f1_score_thresholds}

    preprocessors_dic, operator_names = generate_dic_and_names(component_idx)

    while len(population) < population_size and not time_limit_reached:
        current_epoch += 1


        temp_pipe = [operator_names[0]]
        ...
        try:
            ...
            for f1_threshold in f1_score_thresholds:
                if f1 >= f1_threshold and f1_iterations[f1_threshold] is None:
                    f1_iterations[f1_threshold] = current_epoch
...
f1_score_thresholds = [0.6, 0.65, 0.7, 0.75, 0.8]

#for each compoenent iterations
component_iterations_data = {comp: None for comp in range(6)}

for component_index in range(6):
    population = []
    component_iterations_data[component_index] = pbt_with_population_size(
        population_size, max_time_limit, component_index, f1_score_thresholds)
...
for component, f1_iterations in component_iterations_data.items():
    f1_scores = list(f1_iterations.keys())
    iterations = list(f1_iterations.values())
    
    # Remove None values if any F1 thresholds were not reached
    f1_scores = [f1 for f1, it in zip(f1_scores, iterations) if it is not None]
    iterations = [it for it in iterations if it is not None]
    
    ax.plot(f1_scores, iterations, '-o', label=f'Component {component + 1}')

# Label the plot
ax.set_xlabel('F1 Score')
ax.set_ylabel('Number of Iterations to Reach F1 Score')
ax.set_title('Iterations Required to Reach Different F1 Scores for Each Component')
ax.legend()
ax.grid(True)
